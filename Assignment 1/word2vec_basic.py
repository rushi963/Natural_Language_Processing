...
def generate_batch(data, batch_size, num_skips, skip_window):
    ...
    ### Batch generation moving from left to right
    ...
    #         if num_count == num_skips:
    #             break
    #         if j == 0:
    #             continue
    #         batch[sample_count] = (data[data_index])
    #         labels[sample_count] = (data[data_index + j])
    #         sample_count = sample_count + 1
    #         num_count = num_count + 1
    #     data_index = data_index + 1

    ### Batch generation by moving one step to left and then one step to right and so on from center word

    # Starting data_index from skip_window (start point of center word)
    data_index = data_index + skip_window
    sample_count = 0
    # Run the algorithm till we get batch_size samples
    while sample_count < batch_size:
        num_count = 0
        left = data_index - 1
        right = data_index + 1
        flag = 0
        # If data_index exceeds length of data we start again.
        if data_index > len(data):
            data_index = data_index + skip_window
        while sample_count < batch_size:
            # For each center word if we get num_skips number of samples we go to next center word
            if num_count == num_skips:
                break
            # Go to the left of center word
            if left >= data_index-skip_window and flag == 0:
                batch[sample_count] = data[data_index]
                labels[sample_count] = data[left]
                left = left - 1
                sample_count = sample_count + 1
                num_count = num_count + 1
                flag = 1
                continue
            # Go to the right of the center word
            if right <= data_index+skip_window and flag == 1:
                batch[sample_count] = data[data_index]
                labels[sample_count] = data[right]
                right = right + 1
                sample_count = sample_count + 1
                num_count = num_count + 1
                flag = 0
                continue
        # Incrementing data_index each time
        data_index = data_index + 1
    return batch, labels
...
if __name__ == '__main__':
    ...
    #batch, labels = generate_batch(data, batch_size=8, num_skips=1, skip_window=1)
    #for i in range(8):
